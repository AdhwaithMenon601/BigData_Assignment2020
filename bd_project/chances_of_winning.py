...
def predict(player_chem, player_profile, player_rating, team1, team2):
    ...
    for i in player_coeff1:
    	tot1+=(player_coeff1[i]*predict_regression(i))
    player_coeff2={}
    lessthan2=[]
    for i in team2:
        player = i
    ...
    tot2=0
    for i in player_coeff1:
    	tot2+=(player_coeff1[i]*predict_regression(i))
    ...
    for i in lessthan1:
    	...
    	cluster_id=predictions.filter(predictions["prediction"]==tempdf["prediction"])
	...
	tot=0
	for k in temp_ids1:
		tot+=(player_rating[k]*player_chem[k])
